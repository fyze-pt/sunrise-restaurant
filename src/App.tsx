...
const Navbar = ({ lang, setLang, t }: { lang: 'pt' | 'en', setLang: (l: 'pt' | 'en') => void, t: any }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-sunrise-stone/90 backdrop-blur-md h-20 shadow-lg" : "bg-transparent h-24"}`}>
      <div className="max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm uppercase tracking-widest font-medium">
          ...
          <div className="flex items-center gap-2 border-l border-white/20 pl-8">
            <button 
              onClick={() => setLang('pt')} 
              className={`transition-colors ${lang === 'pt' ? 'text-sunrise-orange' : 'text-white/40 hover:text-white'}`}
            >
              PT
            </button>
            ...
            <button 
              onClick={() => setLang('en')} 
              className={`transition-colors ${lang === 'en' ? 'text-sunrise-orange' : 'text-white/40 hover:text-white'}`}
            >
              EN
            </button>
          </div>
        ...
        <button className="bg-sunrise-orange hover:bg-sunrise-red text-white px-6 py-2 rounded-full text-sm font-bold transition-all duration-300 shadow-lg shadow-sunrise-orange/20">
          {t.nav.reserve}
        </button>
      </div>
    </nav>
    ...
    <section id="inicio" className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div
          ...
          role="img"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-sunrise-stone/60 via-sunrise-stone/40 to-sunrise-stone"></div>
      </div>
      ...
      <div className="relative z-10 text-center px-6 max-w-4xl">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <button className="w-full sm:w-auto bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white px-10 py-4 rounded-full font-bold transition-all duration-300">
              {t.hero.cta_menu}
            </button>
          </div>
        </motion.div>
      ...
      >
        ...
        <div>
          <p className="text-xs uppercase tracking-widest text-white/60">{t.hero.google_review}</p>
          <p className="text-xl font-bold">4.4 {t.hero.stars}</p>
        </div>
      </motion.div>
    </section>
  );
    ...
    <section id="sobre" className="py-24 px-6 bg-sunrise-stone relative overflow-hidden">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-8 mt-12">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-sunrise-wood/20 flex items-center justify-center text-sunrise-orange">
                <Waves className="w-6 h-6" />
              </div>
              <div>
                <h4 className="font-bold">{t.about.feature1_title}</h4>
                <p className="text-sm text-white/50">{t.about.feature1_desc}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-sunrise-wood/20 flex items-center justify-center text-sunrise-orange">
              ...
              <div>
                ...
                <p className="text-sm text-white/50">{t.about.feature2_desc}</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="aspect-[4/5] rounded-3xl overflow-hidden shadow-2xl border-8 border-sunrise-stone-light">
            <img 
              src={restaurantL2} 
          ...
          <div className="absolute -bottom-10 -left-10 w-64 aspect-square rounded-3xl overflow-hidden shadow-2xl border-8 border-sunrise-stone hidden md:block">
            <img 
              src={chefC3} 
              alt="Chef at work" 
              className="w-full h-full object-cover"
    ...
    <section id="menu" className="py-24 px-6 bg-sunrise-stone-light">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-white/60 max-w-2xl mx-auto">{t.menu.description}</p>
        </div>
        ...
        <div className="bg-sunrise-stone p-12 rounded-3xl border border-white/5 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-sunrise-orange/5 blur-3xl rounded-full -mr-32 -mt-32"></div>
          <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h3 className="text-3xl font-serif font-bold mb-6 italic">{t.menu.drinks_title}</h3>
              <p className="text-white/60 mb-8">{t.menu.drinks_desc}</p>
              <div className="grid grid-cols-2 gap-6">
                {t.menu.drinks_list.map((drink: any, i: number) => (
                  <div key={i} className="flex flex-col">
                    <span className="font-bold text-sunrise-orange">{drink.name}</span>
                    <span className="text-xs text-white/40 uppercase tracking-wider">{drink.type}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex items-center gap-6 glass-card p-6">
            <div className="text-center">
              ...
              <div className="flex gap-1 justify-center mt-1">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-3 h-3 ${i < 4 ? "fill-sunrise-orange text-sunrise-orange" : "text-white/20"}`} />
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-white/40 uppercase tracking-widest">{t.reviews.based_on}</p>
            </div>
          </div>
        </div>
    ...
    <section className="py-24 px-6 bg-sunrise-stone-light">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-white/60 max-w-2xl mx-auto">{t.team.description}</p>
        </div>
...
const Footer = ({ t }: { t: any }) => {
  return (
    <footer id="contacto" className="bg-sunrise-stone pt-24 pb-12 px-6 border-t border-white/5">
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-4 gap-12 mb-20">
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-4">
              <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-sunrise-orange transition-colors">
          ...
          <div>
            ...
            <p className="text-white/60 text-sm mb-6">{t.footer.reserve_desc}</p>
        ...
        <div className="h-96 rounded-3xl overflow-hidden grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-700 mb-12">
          <iframe 
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-white/30 text-xs border-t border-white/5 pt-12">
          <p>{t.footer.rights}</p>
